...
declare function m:facets($query as xs:string, $page as xs:int)
{
  let $start := ($page - 1) * $c:DEFAULT-PAGE-LENGTH + 1
  return
    search:search($query, $c:SIDEBAR-OPTIONS, $start, $c:DEFAULT-PAGE-LENGTH)
};

declare function m:document($uri as xs:string)
{
  let $start := 1
  return
    fn:doc($uri)
