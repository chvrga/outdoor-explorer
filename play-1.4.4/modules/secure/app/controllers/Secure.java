...
public class Secure extends Controller {
	protected static boolean bypassCheck() {
		if(	"GET".equals(request.method) ||
			request.url.equals("/createUser")
			) 
		{
			Logger.debug("Security.bypassCheck(): " + request.url);
			return true;
		}
		return false;
	}
    ...
    static void checkAccess() throws Throwable {
		...
		Logger.debug("Secure.checkAccess() for " + request.url);

    	// Authentication
        if(!session.contains("username") && !bypassCheck()) {
            flash.put("url", "GET".equals(request.method) ? request.url : Play.ctxPath + "/"); // seems a good default
            forbidden("You must sign in to perform this action");
    ...
    static void redirectToOriginalURL() throws Throwable {
    	Logger.debug("Secure.redirectToOriginalURL() ");
